    ...
    if (threadId < h*w){
        ...
        if(amin > amax || (amin < 0 || amin == 0)){
            ...
            while((-1 < i && i < (sN[0]-1)) && 
                  ...
                  (-1 < k && k < (sN[2]-1))   ){
                if(ax == minAxyz){
                   d12 = d12 + (ax - ac)*dconv*rho[i + j*(sN[0]-1) + k*(sN[0]-1)*(sN[1]-1)]; 
                   i = (sSrc[0]<dst.x)?(i+1):(i-1);
                   ac = ax;
                   ax = ax + sSp[0]/fabsf(dst.x - sSrc[0]);
                }
                else if(ay == minAxyz){
                   d12 = d12 + (ay - ac)*dconv*rho[i + j*(sN[0]-1) + k*(sN[0]-1)*(sN[1]-1)]; 
                   j = (sSrc[1]<dst.y)?(j+1):(j-1);
                   ac = ay;
                   ay = ay + sSp[1]/fabsf(dst.y - sSrc[1]);
                }
                else {
                   d12 = d12 + (az - ac)*dconv*rho[i + j*(sN[0]-1) + k*(sN[0]-1)*(sN[1]-1)]; 
