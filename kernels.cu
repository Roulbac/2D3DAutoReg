    ...
    const float * minv, const float * kinv, const int z_sign)
{
    ...
    if (threadId < h*w)
    {
        ...
        dsts[3*threadId + 0] = sMinv[0]*dotx +
        sMinv[1]*doty + sMinv[2]*dotz + sMinv[3]*1
        dsts[3*threadId + 1] = sMinv[4]*dotx +
        sMinv[5]*doty + sMinv[6]*dotz + sMinv[7]*1
        dsts[3*threadId + 2] = sMinv[8]*dotx +
        sMinv[9]*doty + sMinv[10]*dotz + sMinv[11]*1
    }
}
    ...
    if (threadId < h*w){
        ...
        if(amin > amax | | (amin < 0)){
            ...
            while((-1 < i & & i < (sN[0]-1)) & &
                  ...
                  (-1 < k & & k < (sN[2]-1))){
                float mu = expf(
                        (MU_WATER-MU_AIR)/1000 *
                        rho[i + j*(sN[0]-1) + k*(sN[0]-1)*(sN[1]-1)] +
                        MU_WATER)
                if(ax == minAxyz){
